...
class MTree(object):
    ...
    def search(self, query_obj, k=1):
        ...
        pr = []
        heappush(pr, PrEntry(self.root, 0, 0)) # Entry of Priority Queue: (tree, dmin, d_query)
...
class NN(object):
    def __init__(self, size):
        self.size = size
        self.elems = []
        self.noneDistances = [float("inf")] * size
    ...
    def search_radius(self):
        ...
        if len(self.elems) == self.size:
            return self.elems[-1].dmax
        else:
            return self.noneDistances[-1]

    def update(self, obj, dmax):
        if obj is None:
            self.noneDistances.append(dmax)
            self.noneDistances.sort()
            self.noneDistances.pop()
        else:
            # Sorted insertion
            self.elems.append(NNEntry(obj, dmax))
            self.elems.sort(key=lambda x: x.dmax)
            if len(self.elems) > self.size:
                self.elems.pop()
    
    def update2(self, obj, dmax):
        if obj is None:
            #internal node
